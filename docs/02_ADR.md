...
**背景**：需要一门具备高并发、高可靠、强类型、生态成熟的语言作为后端主语言，且与本项目"四模块、状态机多、规则强卡控"的特性匹配。

**决策**：使用 **Go 1.22+** + **Gin v1.10+**。

**理由**：
1. 静态类型 + 编译期检查，契合"业务规则强卡控"的诉求；
2. 协程与 channel 原生支持，能轻松应对开学季 5000+ 并发；
3. 标准库与生态成熟，单二进制部署，运维成本低；
4. Gin 性能优秀，路由 + 中间件机制清晰，便于统一鉴权/审计/限流。

**备选方案**：
| 方案 | 优势 | 劣势 | 结论 |
| ---- | ---- | ---- | ---- |
| Java + Spring Boot | 生态最丰富 | 启动慢、内存占用大 | 备选 |
| Python + FastAPI | 开发快 | 性能与类型约束弱 | 备选 |
| Node.js + NestJS | 前后端语言统一 | CPU 密集型场景弱 | 备选 |
| Go + Echo/Fiber | 轻量 | 生态略弱于 Gin | 备选 |

**影响**：
- 所有业务代码须 `go vet` + `staticcheck` 通过；
- 公共库使用 [internal pkg](#31-仓库与目录结构) 模式封装，避免对外暴露。

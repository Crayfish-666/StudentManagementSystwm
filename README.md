...
| 数据库   | SQLite3（[mattn/go-sqlite3](https://github.com/mattn/go-sqlite3)，CGO 绑定） |
| 日志     | [zap](https://github.com/uber-go/zap) v1.27 |
| 鉴权     | [golang-jwt/jwt](https://github.com/golang-jwt/jwt) v5 |
| 校验     | [go-playground/validator](https://github.com/go-playground/validator) v10 |
| 定时任务 | [robfig/cron](https://github.com/robfig/cron) v3 |
| 加密     | `crypto/aes` + `golang.org/x/crypto/bcrypt` |
| 缓存     | [hashicorp/golang-lru](https://github.com/hashicorp/golang-lru) v2（5min TTL，512 条目） |
...
├── backend/                                # Go 后端工程
│   ├── cmd/                                # 可执行入口
│   │   ├── server/main.go                  # 主服务（API + 调度 + 种子）
│   │   ├── dbcheck/main.go                 # 数据库自检工具
│   │   ├── seedstudent/main.go             # 旧版：为学生批量开通 student{NN} 测试账号（已被 SeedStudentUser 回填为学号）
│   │   ├── seedst/main.go                  # 社团活动种子
...
│   │   ├── eventx/bus.go                   # 简易事件总线（CMP 订阅）
│   │   ├── idgen/biz_no.go                 # 业务编号生成器
...
│   │   ├── notifyx/                        # 通知通道（钉钉/企微/邮件/短信）
│   │   ├── scheduler/                      # 定时任务（cron）
│   │   └── statem/state_machine.go         # 通用状态机实现
│   ├── pkg/                                # 通用工具
...
后端监听端口由 `defaultConfig()` 中的 `getEnvInt("APP_PORT", 18080)` 决定（**默认 18080**，项目日常开发常用 **`APP_PORT=8088`** 启动以与日志文件名 `server-8088.log` 对应）。数据库文件位于 `backend/data/studenthub.db`。

```powershell
# 进入后端目录
cd backend

# 方式一：源码直接运行（首次会拉取依赖，默认端口 18080）
go mod tidy
go run ./cmd/server
...
前端开发服务器默认监听 `http://127.0.0.1:5173`，通过 Vite 将 `/api/**` 代理到后端 `http://localhost:8080`，无需手动配置跨域。
...
| 位置 | 命令 | 说明 |
| --- | --- | --- |
| `backend/` | `go run ./cmd/server` | 启动后端服务（监听 `:8080`） |
...
| `backend/` | `go run ./cmd/seedst` | 社团活动示例数据 |
...
| 位置 | 命令 | 说明 |
| --- | --- | --- |
| `frontend/` | `pnpm dev` | 启动 Vite 开发服务器（`http://127.0.0.1:5173`） |
...
- 前端 `vite.config.js` 已将 `/api` 代理到 `:8080`，**开发期请保持后端在运行**。
...
A：检查后端是否已启动并监听 `:8080`；浏览器打开 `http://localhost:8080/api/v1/healthz` 应返回 `{"status":"healthy"}`。Vite 代理目标见 [`vite.config.js`](file:///d:/Teach/AI_Coding/StudentHub/frontend/vite.config.js)。

**Q3：登录成功但部分菜单点击提示 403。**
A：当前账号未授予对应角色。管理员账号拥有全部权限（`R-SY-ADMIN`）；学生账号仅 `R-STU-NORM` 自助权限。可在「系统管理 → 用户管理」中调整角色绑定。

**Q4：修改了 `config.yaml` 不生效？**
A：当前实现以 [`boot.defaultConfig()`](file:///d:/Teach/AI_Coding/StudentHub/backend/internal/boot/boot.go#L77-L89) 内的默认值为准，仅 `APP_ENV` 与 `JWT_SECRET` 支持环境变量覆盖。如需扩展其他配置项，请在 `boot.go` 中补充 `getEnv` 调用并同步文档。
